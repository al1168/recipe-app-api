...
class PrivateRecipeAPITests(TestCase):
    ...
    def test_filter_by_tags(self):
        ...
        r2 = create_recipe(user=self.user, title='Aubergine with Tahnini')
    ...
    def test_filter_by_ingredients(self):  # need to add back test
        """Test filter recipes by ingredients"""
        ingredient_1 = Ingredient.objects.create(user=self.user, name="mango")
        ingredient_2 = Ingredient.objects.create(user=self.user, name="apple")

        r1 = create_recipe(user=self.user, title='cool mango')
        r2 = create_recipe(user=self.user, title="Def not mangos")
        r3 = create_recipe(user=self.user, title="apple mango")
        params = {'ingredients': f'{ingredient_1.id},{ingredient_2.id}'}
        r1.ingredients.add(ingredient_1)
        r3.ingredients.add(ingredient_1)
        r3.ingredients.add(ingredient_2)
